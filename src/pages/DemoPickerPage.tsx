...
export function DemoPickerPage() {
  ...
  useEffect(() => {
    posthog?.capture('demo_viewed', { doc_type: 'picker', path: '/demo' })
    posthog?.capture('demo_picker_viewed')
  }, [posthog])

  function captureSignupClick() {
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 24px 64px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
          {DOC_TYPES.map(type => (
            <Link
              key={type.type}
              to={`/demo/${type.type}?source=demo_picker`}
              onClick={() => posthog?.capture('demo_document_selected', { doc_type: type.type })}
              style={{
