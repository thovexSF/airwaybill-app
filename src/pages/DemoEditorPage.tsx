...
export function DemoEditorPage() {
  ...
  const signupUrl = `/signup?source=demo&intent=download_pdf&doc_type=${demoDocType}`
    ...
    <div className="app sheet-editor">
      <div style={{
        ...
        <Link
          ...
          onClick={() => posthog?.capture('demo_signup_cta_clicked', { placement: 'banner', doc_type: demoDocType, intent: 'download_pdf' })}
        >
          {t('demo.signupCta')} →
        </Link>
      </div>
      ...
      <div className="action-bar" style={{ background: '#6b0000', borderBottom: '1px solid rgba(255,255,255,0.1)', padding: '0 20px', height: 38, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <Link
            to={signupUrl}
            state={{ from: `/demo/${demoDocType}`, source: 'demo', intent: 'download_pdf', doc_type: demoDocType }}
            className="btn-download"
            onClick={() => posthog?.capture('demo_signup_cta_clicked', { placement: 'download_button', doc_type: demoDocType, intent: 'download_pdf' })}
          >
            {t('demo.downloadCta')}
          </Link>
        </div>
