...
export function SignupPage() {
  ...
  const attribution = useMemo(() => {
    ...
    const source = params.get('source') ?? state?.source ?? (fromPath ? fromPath.replace(/^\//, '') : 'direct')
    const intent = params.get('intent') ?? state?.intent ?? (source === 'demo' ? 'download_awb_pdf' : null)

    return { source, intent, fromPath }
  }, [location.search, location.state])

  const signupProperties = (extra?: Record<string, unknown>) => ({
    source: attribution.source,
    intent: attribution.intent,
    from_path: attribution.fromPath,
    ...extra,
  })

  useEffect(() => {
    ;(window as any).clarity?.('event', 'signup_page_viewed')
    posthog?.capture('signup_page_viewed', signupProperties())
  }, [posthog, attribution.source, attribution.intent, attribution.fromPath])
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault()
    setError('')
    posthog?.capture('signup_attempted', signupProperties({
      method: 'email',
    ...
    const validationError = !trimmedEmail
      ? { code: 'missing_email', message: t('auth.signup.errors.emailRequired') }
      : !emailLooksValid
        ? { code: 'invalid_email', message: t('auth.signup.errors.emailInvalid') }
        : !password
          ? { code: 'missing_password', message: t('auth.signup.errors.passwordRequired') }
          : password.length < 6
            ? { code: 'short_password', message: t('auth.signup.errors.passwordShort') }
            : null

    if (validationError) {
      setError(validationError.message)
      posthog?.capture('signup_validation_failed', signupProperties({
        method: 'email',
    ...
    setLoading(true)
    const result = await signup({ companyName: trimmedCompanyName, email: trimmedEmail, password })
    setLoading(false)
    if (!result.ok) {
      setError(result.error)
      posthog?.capture('signup_failed', signupProperties({
        method: 'email',
    ...
    posthog?.capture('user_signed_up', signupProperties({
      method: 'email',
      company_provided: trimmedCompanyName.length > 0,
    }))
    navigate('/editor', { replace: true })
  }

  function handleProviderSignup(provider: 'google' | 'github') {
    sessionStorage.setItem('posthog_pending_signup_provider', provider)
    sessionStorage.setItem('posthog_pending_signup_source', attribution.source)
    if (attribution.intent) sessionStorage.setItem('posthog_pending_signup_intent', attribution.intent)
    if (attribution.fromPath) sessionStorage.setItem('posthog_pending_signup_from_path', attribution.fromPath)
    ;(window as any).clarity?.('event', `signup_with_${provider}`)
    posthog?.capture('signup_provider_clicked', signupProperties({ provider }))
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <ul className="auth-benefits">
          <li>{t('auth.signup.benefits.download')}</li>
          <li>{t('auth.signup.benefits.noCard')}</li>
          <li>{t('auth.signup.benefits.editor')}</li>
        </ul>
        ...
        <form className="auth-form" onSubmit={handleSubmit} noValidate>
          ...
          <div className="auth-field">
            ...
            <div className="auth-helper">{t('auth.signup.passwordHelp')}</div>
          </div>
...
function categorizeSignupError(error: string) {
  const normalized = error.toLowerCase()
  if (normalized.includes('already') || normalized.includes('registered')) return 'account_exists'
