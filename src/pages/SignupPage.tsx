...
export function SignupPage() {
  ...
  const routeState = location.state as { from?: string; intent?: string } | null
  const fromDemo = routeState?.from === '/demo' || searchParams.get('from') === 'demo'
  const signupSource = fromDemo ? 'demo' : 'direct'
  ...
  useEffect(() => {
    posthog?.capture('signup_page_viewed', { source: signupSource })
  }, [posthog, signupSource])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    posthog?.capture('user_signed_up', { method: 'email', company_name: companyName, source: signupSource })
    navigate('/editor', { replace: true })
  }

  function handleProviderSignup(provider: 'google' | 'github') {
    ;(window as any).clarity?.('event', `signup_with_${provider}`)
    posthog?.capture('user_signed_up_with_provider', { provider, source: signupSource })
      ...
      <div className="auth-card">
        ...
        {fromDemo && (
          <div className="auth-demo-callout">
            <strong>{t('auth.signup.demoCalloutTitle')}</strong>
            <span>{t('auth.signup.demoCalloutSub')}</span>
          </div>
        )}

        <ul className="auth-trust-list">
          <li>{t('auth.signup.trustFree')}</li>
          <li>{t('auth.signup.trustNoCard')}</li>
          <li>{t('auth.signup.trustContinue')}</li>
        </ul>
