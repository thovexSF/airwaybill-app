...
function signupErrorType(message: string): string {
  ...
  if (normalized.includes('already') || normalized.includes('registered')) return 'already_registered'
...
export function SignupPage() {
  ...
  const intent = params.get('intent') ?? 'create_account'
  const docType = params.get('doc_type')
  const from = locationState?.from
  const attribution = { source, intent, doc_type: docType, from }

  useEffect(() => {
    posthog?.capture('signup_page_viewed', attribution)
  }, [docType, from, intent, posthog, source])
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <div className="auth-social">
          <button className="auth-social-btn google" onClick={() => { (window as any).clarity?.('event', 'signup_with_google'); posthog?.capture('signup_provider_clicked', { ...attribution, provider: 'google' }); loginWithProvider('google') }}>
          ...
          <button className="auth-social-btn github" onClick={() => { (window as any).clarity?.('event', 'signup_with_github'); posthog?.capture('signup_provider_clicked', { ...attribution, provider: 'github' }); loginWithProvider('github') }}>
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          <div className="auth-field">
            ...
            <input type="email" value={email} onChange={e => { markFormStarted(); setEmail(e.target.value) }} placeholder="operaciones@empresa.com" />
          </div>
          <div className="auth-field">
            <label>{t('auth.signup.password')}</label>
            <input type="password" value={password} onChange={e => { markFormStarted(); setPassword(e.target.value) }} placeholder="••••••" />
          </div>
