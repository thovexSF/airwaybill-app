...
export function SignupPage() {
  ...
  const source = (location.state as { from?: string } | undefined)?.from ?? new URLSearchParams(location.search).get('from') ?? 'direct'
  ...
  useEffect(() => {
    ...
    posthog?.capture('signup_page_viewed', { source })
  }, [posthog, source])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError('')
    setLoading(true)
    ;(window as any).clarity?.('event', 'signup_email_attempted')
    ...
    if (!result.ok) {
      ...
      posthog?.capture('signup_failed', { method: 'email', source, error_message: result.error })
      return
    }
    ;(window as any).clarity?.('event', 'signup_completed')
    posthog?.capture('user_signed_up', { method: 'email', source, company_name: companyName })
    navigate('/editor', { replace: true })
  }

  async function handleProvider(provider: 'google' | 'github') {
    ;(window as any).clarity?.('event', `signup_with_${provider}`)
    posthog?.capture('signup_provider_clicked', { provider, source })
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <ul className="auth-proof-list">
          <li>{t('auth.signup.proof.noCard')}</li>
          <li>{t('auth.signup.proof.downloads')}</li>
          <li>{t('auth.signup.proof.afterSignup')}</li>
        </ul>
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          <p className="auth-microcopy">{t('auth.signup.microcopy')}</p>
        </form>
