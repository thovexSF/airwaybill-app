...
import { authErrorCode, getFunnelContext, viewportProps } from '../lib/funnelAnalytics'
...
export function SignupPage() {
  ...
  const signupContext = getFunnelContext(location.search, location.pathname)
  ...
  const validationTrackedRef = useRef<Set<string>>(new Set())
  ...
  function markFormStarted(field: string) {
  ...
  function trackValidationBlocked(field: string) {
    if (validationTrackedRef.current.has(field)) return
    validationTrackedRef.current.add(field)
    track('signup_validation_failed', {
      ...signupContext,
      field,
    })
  }
  ...
  async function handleProvider(provider: 'google' | 'github') {
    ...
    track('signup_provider_clicked', {
      ...signupContext,
      provider,
    })
    sessionStorage.setItem('posthog_pending_signup_context', JSON.stringify({
      ...signupContext,
      provider,
    }))
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    track('signup_attempted', {
      ...signupContext,
      method: 'email',
      has_company_name: Boolean(companyName.trim()),
    })
    ...
    if (!result.ok) {
      ...
      track('signup_failed', {
        ...signupContext,
        method: 'email',
        error_code: authErrorCode(result.error),
      })
      return
    }
      ...
      <div className="auth-card">
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          <div className="auth-field">
            ...
            <input value={companyName} onChange={e => { markFormStarted('company'); setCompanyName(e.target.value) }} placeholder={t('auth.signup.companyPlaceholder')} autoComplete="organization" />
          </div>
          <div className="auth-field">
            ...
            <input
              ...
              onInvalid={() => trackValidationBlocked('email')}
          ...
          <div className="auth-field">
            ...
            <input
              ...
              onInvalid={() => trackValidationBlocked('password')}
              placeholder="••••••"
