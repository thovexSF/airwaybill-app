...
function safeAttributionValue(value: string | null | undefined) {
  const trimmed = value?.trim()
  if (!trimmed) return undefined
  return /^[a-z0-9_/-]+$/i.test(trimmed) ? trimmed.slice(0, 80) : undefined
}
...
export function SignupPage() {
  ...
  const attribution = useMemo<SignupAttribution>(() => {
    ...
    const from = safeAttributionValue(stateFrom)
    return {
      source: safeAttributionValue(params.get('source')) ?? (from?.startsWith('/demo') ? 'demo' : undefined),
      intent: safeAttributionValue(params.get('intent')),
      doc_type: safeAttributionValue(params.get('doc_type')),
      from,
    }
  ...
  async function handleProvider(provider: 'google' | 'github') {
    ...
    posthog?.capture('signup_provider_clicked', { provider, ...attribution })
    sessionStorage.setItem('posthog_pending_signup', JSON.stringify({ method: provider, ...attribution }))
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <div className="auth-social">
          <button className="auth-social-btn google" onClick={() => { void handleProvider('google') }}>
          ...
          <button className="auth-social-btn github" onClick={() => { void handleProvider('github') }}>
