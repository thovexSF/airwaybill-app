...
export function SignupPage() {
  ...
  const locationState = location.state as { from?: string; intent?: string } | null
  const searchParams = new URLSearchParams(location.search)
  const signupSource = searchParams.get('source') ?? (locationState?.from === '/demo' ? 'demo' : 'direct')
  const signupIntent = searchParams.get('intent') ?? locationState?.intent ?? null
  const isDemoSignup = signupSource === 'demo'
  ...
  }, [posthog, signupSource, signupIntent])
  ...
  async function handleProvider(provider: 'google' | 'github') {
    ...
    posthog?.capture('user_signed_up_with_provider', {
      provider,
      source: signupSource,
      intent: signupIntent,
    })
  ...
  return (
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        {isDemoSignup && (
          <div className="auth-intent-card">
            <strong>{t('auth.signup.demoIntro.title')}</strong>
            <p>{t('auth.signup.demoIntro.sub')}</p>
            <ul>
              <li>{t('auth.signup.demoIntro.save')}</li>
              <li>{t('auth.signup.demoIntro.downloads')}</li>
              <li>{t('auth.signup.demoIntro.noCard')}</li>
            </ul>
          </div>
        )}
