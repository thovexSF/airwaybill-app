...
export function SignupPage() {
  ...
  const eventProps = useMemo<SignupEventProps>(() => {
    const params = new URLSearchParams(location.search)
    const state = (location.state as SignupLocationState | null) ?? {}
    const fromDemo = state.from === '/demo'

    return {
      source: params.get('source') ?? state.source ?? (fromDemo ? 'demo' : 'direct'),
      intent: params.get('intent') ?? state.intent ?? (fromDemo ? 'download_awb_pdf' : 'create_account'),
    }
  }, [location.search, location.state])

  useEffect(() => {
    posthog?.capture('signup_page_viewed', eventProps)
  }, [posthog, eventProps])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError('')
    setLoading(true)
    const companyProvided = companyName.trim().length > 0
    ...
    setLoading(false)
    if (!result.ok) {
      const errorCategory = categorizeSignupError(result.error)
  ...
  function handleInvalid(field: 'email' | 'password') {
    posthog?.capture('signup_form_invalid', {
      ...eventProps,
      field,
    })
  }
  ...
  async function handleProvider(provider: 'google' | 'github') {
    ...
    posthog?.capture('signup_provider_clicked', {
      ...eventProps,
      provider,
    })
    await loginWithProvider(provider, { flow: 'signup', ...eventProps })
  }
  ...
  return (
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <p className="auth-value-prop">{t('auth.signup.valueProp')}</p>
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          <div className="auth-field">
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="operaciones@empresa.com" autoComplete="email" required onInvalid={() => handleInvalid('email')} />
          </div>
          <div className="auth-field">
            <label>{t('auth.signup.password')}</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••" autoComplete="new-password" minLength={6} required onInvalid={() => handleInvalid('password')} />
            <span className="auth-hint">{t('auth.signup.passwordHint')}</span>
          </div>
...
function categorizeSignupError(message: string) {
  const lower = message.toLowerCase()
  if (lower.includes('already') || lower.includes('registered') || lower.includes('exists')) return 'account_exists'
