...
export function SignupPage() {
  ...
  const signupSource = searchParams.get('source') ?? locationState?.source ?? (locationState?.from === '/demo' ? 'demo' : 'direct')
  const signupIntent = searchParams.get('intent') ?? locationState?.intent ?? 'create_account'
  const analyticsProps = useMemo(() => ({
    source: signupSource,
    intent: signupIntent,
  }), [signupIntent, signupSource])

  useEffect(() => {
    posthog?.capture('signup_page_viewed', analyticsProps)
  }, [analyticsProps, posthog])
