...
function signupFailureReason(error: string): string {
...
export function SignupPage() {
  ...
  const attribution = useMemo(() => {
    const from = searchParams.get('from') ?? navState?.from ?? undefined
    const source =
    ...
    return {
      source,
      intent: searchParams.get('intent') ?? navState?.intent ?? undefined,
      doc_type: searchParams.get('doc_type') ?? navState?.doc_type ?? undefined,
      from,
    }
  }, [navState?.doc_type, navState?.from, navState?.intent, navState?.source, searchParams])
