...
export function SignupPage() {
  ...
  useEffect(() => {
    posthog?.capture('signup_page_viewed', { source, intent })
  }, [intent, posthog, source])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(false)
    if (!result.ok) {
      posthog?.capture('signup_failed', { method: 'email', source, intent, error: result.error })
  ...
  async function handleProvider(provider: 'google' | 'github') {
    ...
    posthog?.capture('user_signed_up_with_provider', { provider, source, intent })
