...
export function SignupPage() {
  ...
  useEffect(() => {
    posthog?.capture('signup_page_viewed', authEventProps(attribution))
  }, [attribution, posthog])
  ...
  function trackFormStart() {
    ...
    posthog?.capture('signup_form_started', authEventProps(attribution))
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    posthog?.capture('signup_attempted', authEventProps(attribution, {
      method: 'email',
    ...
    if (!result.ok) {
      ...
      posthog?.capture('signup_failed', authEventProps(attribution, {
        method: 'email',
    ...
    posthog?.capture('user_signed_up', authEventProps(attribution, {
      method: 'email',
      company_provided: companyProvided,
    }))
  ...
  async function handleProvider(provider: 'google' | 'github') {
    ...
    posthog?.capture('signup_provider_clicked', authEventProps(attribution, { provider }))
    const result = await loginWithProvider(provider, { flow: 'signup', attribution })
    if (!result.ok) {
      setError(result.error)
      posthog?.capture('signup_provider_failed', authEventProps(attribution, {
        provider,
        reason: classifyAuthError(result.error),
      }))
    }
      ...
      <div className="auth-card">
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          <div className="auth-field">
            ...
            <input value={companyName} onChange={e => { trackFormStart(); setCompanyName(e.target.value) }} placeholder={t('auth.signup.companyPlaceholder')} />
          </div>
          <div className="auth-field">
            <label>{t('auth.signup.email')}</label>
            <input type="email" value={email} onChange={e => { trackFormStart(); setEmail(e.target.value) }} placeholder="operaciones@empresa.com" />
          </div>
          <div className="auth-field">
            <label>{t('auth.signup.password')}</label>
            <input type="password" value={password} onChange={e => { trackFormStart(); setPassword(e.target.value) }} placeholder="••••••" />
          </div>
