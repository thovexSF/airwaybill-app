...
const SIGNUP_SOURCES = new Set(['direct', 'demo', 'landing', 'pricing'])
const SIGNUP_INTENTS = new Set(['create_account', 'create_first_awb', 'download_pdf', 'upgrade'])
const DEMO_DOC_TYPES = new Set(['awb', 'hawb', 'dgd', 'manifest', 'label', 'proforma', 'bl', 'bl_manifest', 'imo_dgd', 'neppex', 'fwb', 'fhl', 'ffr'])

function knownParam(value: string | null, allowed: Set<string>, fallback: string) {
  if (!value) return fallback
  const normalized = value.toLowerCase()
  return allowed.has(normalized) ? normalized : fallback
}

function optionalKnownParam(value: string | null, allowed: Set<string>) {
  if (!value) return undefined
  const normalized = value.toLowerCase()
  return allowed.has(normalized) ? normalized : undefined
}

function signupErrorCode(error: string) {
  const normalized = error.toLowerCase()
  if (normalized.includes('already')) return 'already_registered'
...
export function SignupPage() {
  ...
  const source = knownParam(searchParams.get('source'), SIGNUP_SOURCES, 'direct')
  const intent = knownParam(searchParams.get('intent'), SIGNUP_INTENTS, 'create_account')
  const docType = optionalKnownParam(searchParams.get('doc_type'), DEMO_DOC_TYPES)
  const signupContext = { source, intent, ...(docType ? { doc_type: docType } : {}) }

  useEffect(() => {
    posthog?.capture('signup_page_viewed', signupContext)
  }, [source, intent, docType, posthog])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    posthog?.capture('signup_attempted', {
      method: 'email',
      ...signupContext,
      has_company: companyName.trim().length > 0,
    })
    ...
    if (!result.ok) {
      posthog?.capture('signup_failed', {
        method: 'email',
        ...signupContext,
        error_code: signupErrorCode(result.error),
      })
  ...
  async function handleProvider(provider: 'google' | 'github') {
    ...
    posthog?.capture('signup_provider_clicked', { provider, ...signupContext })
    sessionStorage.setItem('posthog_pending_signup', JSON.stringify({
      method: provider,
      ...signupContext,
    }))
